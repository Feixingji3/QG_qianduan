...
router.get('/', checkLogin, async (req, res) => {
    try {
        ...
        // 学生权限检查：如果传了studentId但不是自己，拒绝访问
        if (user.role === 'student' && studentId && parseInt(studentId) !== user.id) {
            return res.json({ code: 403, message: '无权限查看他人成绩', data: null })
        }
        ...
        if (user.role === 'head_teacher') {
            ...
            // 如果传了classId，检查是否有权限访问
            if (classId && !classIds.includes(parseInt(classId))) {
                return res.json({ code: 403, message: '无权限查看该班级', data: null })
            }
            ...
            if (classIds.length > 0) {
                ...
                return res.json({ code: 403, message: '您没有管理任何班级', data: null })
            }
        }
...
router.post('/', checkLogin, async (req, res) => {
    try {
        ...
        // 权限校验：只有班主任和主任可以录入成绩
        if (user.role === 'student') {
            return res.json({ code: 403, message: '无权限录入成绩', data: null })
        }
        ...
        if (user.role === 'head_teacher') {
            ...
            if (!classIds.includes(parseInt(classId))) {
                return res.json({ code: 403, message: '无权限为该班级录入成绩', data: null })
            }
        }
...
router.put('/:id', checkLogin, async (req, res) => {
    try {
        ...
        if (user.role === 'head_teacher') {
            ...
            if (classIds.length === 0) {
                return res.json({ code: 403, message: '无权限修改成绩', data: null })
            }
        ...
        const [result] = await db.query(
            `UPDATE scores SET score = ?, updated_by = ?, updated_at = NOW() ${whereClause}`,
            [score, user.id, ...params]
        )

        if (result.affectedRows === 0) {
            return res.json({ code: 403, message: '无权限修改该成绩或成绩不存在', data: null })
        }
...
router.delete('/:id', checkLogin, async (req, res) => {
    try {
        ...
        if (user.role === 'head_teacher') {
            ...
            if (classIds.length === 0) {
                return res.json({ code: 403, message: '无权限删除成绩', data: null })
            }
        ...
        const [result] = await db.query(
            `UPDATE scores SET status = 0 ${whereClause}`,
            params
        )

        if (result.affectedRows === 0) {
            return res.json({ code: 403, message: '无权限删除该成绩或成绩不存在', data: null })
        }
    ...
    try {
        ...
        if (user.role === 'student') {
            ...
            if (classIds.length > 0) {
                ...
                // 没有班级的班主任，返回只有表头的空CSV
                const csvContent = '\uFEFF学生姓名,班级,科目,考试名称,分数,考试日期\n'
...
router.post('/import', checkLogin, async (req, res) => {
    try {
        ...
        if (user.role === 'head_teacher') {
            ...
            allowedClassIds = teacherClasses.map(tc => parseInt(tc.class_id))
            
            if (allowedClassIds.length === 0) {
                return res.json({ code: 403, message: '您没有管理任何班级，无法导入成绩', data: null })
            }
        }
        
        // ========== 新增：学生验证准备 ==========
        const [allStudents] = await db.query(
            'SELECT id, real_name, username FROM users WHERE role = ? AND status = 1',
            ['student']
        )
        ...
        for (let i = 0; i < scores.length; i++) {
            ...
            try {
                // ========== 新增：班级权限验证（仅班主任）==========
                const classId = parseInt(score.classId)
                
                if (user.role === 'head_teacher' && allowedClassIds) {
                    if (!allowedClassIds.includes(classId)) {
                        failCount++
                        errors.push(`第${rowNum}行：班级不在您的管理范围内`)
                        continue
                    }
                }
                
                // ========== 新增：学生存在性验证 ==========
                const studentKey = String(score.studentId || score.studentUserId)
                const student = studentMap[studentKey]
                
                if (!student) {
                    failCount++
                    errors.push(`第${rowNum}行：学号 ${score.studentId} 的学生不存在`)
                    continue
                }
                
                // ========== 新增：姓名匹配验证 ==========
                if (score.studentName && score.studentName.trim() !== student.real_name) {
                    failCount++
                    errors.push(`第${rowNum}行：学号与姓名不匹配，该学号对应的是"${student.real_name}"`)
                    continue
                }
                ...
                const [existing] = await db.query(
                    'SELECT id, status FROM scores WHERE student_user_id = ? AND class_id = ? AND subject_id = ? AND exam_name = ?',
                    [finalStudentId, classId, score.subjectId, score.examName]
                )
                ...
                if (existing.length > 0) {
                    if (existing[0].status === 0) {
                        ...
                        // 已存在且未删除，跳过
                        failCount++
                        errors.push(`第${rowNum}行：该学生此科目本次考试成绩已存在`)
                    }
                    continue
                }
                ...
                await db.query(
                    `INSERT INTO scores (student_user_id, class_id, subject_id, score, exam_name, exam_date, created_by, updated_by, created_at, updated_at)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
                    [finalStudentId, classId, score.subjectId, score.score, score.examName, score.examDate || new Date(), user.id, user.id]
                )
                successCount++
            } catch (e) {
