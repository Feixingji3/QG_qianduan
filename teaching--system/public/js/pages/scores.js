...
function validateCachedFilters() {
    ...
    // 校验班级ID是否有效
    if (scorePageState.filters.classId) {
        const validClassIds = scorePageState.classes.map(c => String(c.id));
        if (!validClassIds.includes(String(scorePageState.filters.classId))) {
            console.log('缓存的班级已不存在，清除班级筛选条件:', scorePageState.filters.classId);
            scorePageState.filters.classId = '';
            hasInvalidFilter = true;
        }
    }
    
    // 校验科目ID是否有效
    if (scorePageState.filters.subjectId) {
        const validSubjectIds = scorePageState.subjects.map(s => String(s.id));
        if (!validSubjectIds.includes(String(scorePageState.filters.subjectId))) {
            console.log('缓存的科目已不存在，清除科目筛选条件:', scorePageState.filters.subjectId);
            scorePageState.filters.subjectId = '';
            hasInvalidFilter = true;
        }
    }
...
function handleContentClick(e) {
    ...
    if (window.currentPageId !== 'scores') return;
