...
function handleContentClick(e) {
    ...
    if (window.currentPageId !== 'notices') return;
...
function renderNoticeList() {
    ...
    noticePageState.list.forEach(item => {
        ...
        const card = NotificationCard.createDetailed({
            id: item.id,
            title: item.title,
        ...
        if (shouldShowRead(item, role, id)) card.element.classList.add('read');
...
/**
 * 校验缓存的筛选条件是否有效
 * 如果缓存的班级已不存在，则清空对应的筛选条件
 */
function validateCachedFilters() {
    // 只有教务主任使用班级筛选，且只有教务主任加载班级列表
    if (currentUser?.role !== ROLE_DIRECTOR) return;
    
    // 校验班级ID是否有效
    if (noticePageState.filters.classId) {
        const validClassIds = noticePageState.classes.map(c => String(c.id));
        if (!validClassIds.includes(String(noticePageState.filters.classId))) {
            console.log('缓存的班级已不存在，清除班级筛选条件:', noticePageState.filters.classId);
            noticePageState.filters.classId = '';
            // 更新本地缓存
